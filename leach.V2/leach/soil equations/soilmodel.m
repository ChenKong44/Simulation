function [alpha, beta] = soilmodel(moisture)
...
% Ver 1. 2/2013
   ...
   %DC of free water
   ep_fwprr = (2.* pi.* freq.* tau_w.* ( ep_w0- ep_win ))./ (1+ ( 2.* pi.* freq.* tau_w ).^2) + ( mue_eff.* (ro_s-ro_b)./ ( 2.*pi .* ep_0.* freq.* ro_s.* m_v ) );
   %LF of free water
   
   b_pr = ( 127.8- 0.519.*S- 0.152.*C )./ 100;
   b_prr = ( 1.33797 - 0.603.*S- 0.166.*C )./100;
   
   ep_pr = 1 + ( ro_b./ ro_s ).* (ep_s.^0.65-1)+ ( m_v.^b_pr ).* ( ep_fwpr.^0.65 )- m_v;
