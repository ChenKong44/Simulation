function [alpha, beta] = soilmodel(moisture, numNode)
...
% Ver 1. 2/2013
   ...
   ep_0 = 8.854.* 10e-12;   %free-space permittivity
   mu_0 = 0.5;              %permeability in vacuum
   mu_r = 1;              %soil relative permeability
    
   freq = 2.4.*10e8;    %frequency
   ro_b = 5;            %bulk density in grams per cubic centimeter
   ro_s = 6;            %specific gravity of the solid soil particles
   ...
   tau_w = 0.58.* (10e-10)./ (2.*pi);                      %relaxation time for free water
   ...
   %DC of free water
   ep_fwprr = (2.* pi.* freq.* tau_w.* ( ep_w0- wp_win ))./ (1+ ( 2.* pi.* freq.* tau_w ).^2) + ( mue_eff.* (ro_s-ro_b)./ ( 2.*pi .* ep_0.* freq.* ro_s.* m_v ) );
   %LF of free water
   
   b_pr = ( 127.8- 0.519.*S- 0.152.*C )./ 100;
   b_prr = ( 1.33797 - 0.603.*S- 0.166.*C )./100;
   
   ep_pr = 1 + ( rp_b./ ro_s ).* (ep_s.^0.65-1)+ ( m_v.^b_pr ).* ( ep_fwpr.^0.65 )- m_v;
