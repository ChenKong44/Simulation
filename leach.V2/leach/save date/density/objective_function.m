...
% L_result = subs(L_expect(z),z,z_spare2_15);
% L_result1 = subs(L_expect(z),z,z_spare2_45new);
% L_result2 = subs(L_expect(z),z,z_spare2_55);
% L_result3 = subs(L_expect(z),z,z_spare2_ori);
% 
% syms a b
% h_constraint(a,b) = 3./2.*(sqrt(a./4./(density1))+sqrt(b./4./(density1)))-coverage;
% h_result = subs(h_constraint,{a,b},{round(z_spare2_15),z_spare_15});
% h_result1 = subs(h_constraint,{a,b},{round(z_spare2_45new),z_spare_45new});
% h_result2 = subs(h_constraint,{a,b},{round(z_spare2_55),z_spare_55});

 
z=1:1:3000;
x = 1:1:2000;

plot(z, H_spare_ori, 'k-', 'LineWidth', 2); % Plot fitted line.

hold on;
plot(x, H_spare_15, 'g-', 'LineWidth', 2); % Plot fitted line.

hold on;
plot(z, H_spare_45new, 'r-', 'LineWidth', 2); % Plot fitted line.

hold on;
plot(z, H_spare_55, 'b-', 'LineWidth', 2);
% 
% hold on;
